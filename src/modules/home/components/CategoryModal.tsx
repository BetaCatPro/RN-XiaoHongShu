...
export default forwardRef((props: Props, ref) => {
    ...
    useEffect(() => {
        ...
        const list1 = categoryList.filter(i => (i as any).isAdd)
        const list2 = categoryList.filter(i => !(i as any).isAdd)
                ...
                <View style={styles.listContent}>
                    {myList.map((item: Category, index: number) => {
                        return (
                            <TouchableOpacity
                                key={`${item.name}`}
                                style={(item as any).default ? styles.itemLayoutDefault : styles.itemLayout}
                                onPress={onMyItemPress(item, index)}
                            >
                                <Text style={styles.itemTxt}>{item.name}</Text>
                                {edit && !(item as any).default &&
